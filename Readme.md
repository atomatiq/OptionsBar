...
Нам понадобится библиотека `AdWindows.dll`, официальная и устанавливается вместе с Revit. Мы подключим Nuget пакет, чтобы проект компилировался на любом устройстве.
...
Для доступа к ленте в Revit воспользуемся статическим свойством `ComponentManager.Ribbon`, из библиотеки `AdWindows.dll` которую мы подключили ранее. Оно необходимо для получения ссылки на OptionsBar.
